...
-doc "An UUID specification".
-export_type([uuid_spec/0]).
-type uuid_spec() :: uuid_short() | uuid_map().

-doc "A shorthand for a UUID specification".
-type uuid_short() ::
    uuid_nil()
    | uuid_max()
    | uuid_short_v4()
...
-type uuid_nil() :: nil.
-export_type([uuid_nil/0]).

%--- UUID - max

-doc "The 'max' uuid (`ffffffff-ffff-ffff-ffff-ffffffffffff`)".
-type uuid_max() :: max.
-export_type([uuid_max/0]).

%--- UUID - v4

-doc "UUID version 4 shorthand (with optional random components)".
-type uuid_v4() :: #{
    rand_a => uuid_rand_48(),
    rand_b => uuid_rand_12(),
    rand_c => uuid_rand_62()
...
-type uuid_short_v4() ::
    4
    | {4, uuid_rand_48(), uuid_rand_12(), uuid_rand_62()}.
-export_type([uuid_short_v4/0]).

%--- UUID - v7

-doc "UUID version 7 (with optional timestamp and random components)".
-type uuid_v7() :: #{
    unix_ts => uuid_v7_unix_ts_ms(),
    rand_a => uuid_rand_12(),
    rand_b => uuid_rand_62()
}.
-export_type([uuid_v7/0]).

-doc "UUID version 7 shorthand (with optional timestamp and random components)".
-type uuid_short_v7() ::
    7
    | {7, uuid_v7_unix_ts_ms()}
    | {7, uuid_v7_unix_ts_ms(), uuid_rand_12(), uuid_rand_62()}.
-export_type([uuid_short_v7/0]).

-doc """
UNIX timestamp in millisecond resolution.

(Tagged) timestamp value as generated from
[`erlang:system_time(millisecond)`](https://www.erlang.org/doc/apps/erts/erlang.html#system_time/1).
""".
-type uuid_v7_unix_ts_ms() ::
    non_neg_integer()
    | {time_unit(), non_neg_integer()}.
